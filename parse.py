...
def disasm_from_raw(settings, address, num_insns):
    return settings['capstone'].disasm(settings['binary'][settings['text-offset'] + address:], address, num_insns)
...
def read_num_args(settings, address):
    return ptrutil.read_half_word(settings, settings['text-offset'] + address - settings['halfword-size']*5)
...
def read_closure_type(settings, address):
    ...
    type = ptrutil.read_half_word(settings, settings['text-offset'] + address - settings['halfword-size']*2)
...
def read_closure(settings, parsed, pointer):
    try:
        ...
        if info_type[:11] == 'constructor':
            num_ptrs = ptrutil.read_half_word(settings, settings['text-offset'] + info_pointer.value - settings['halfword-size']*4)
            num_non_ptrs = ptrutil.read_half_word(settings, settings['text-offset'] + info_pointer.value - settings['halfword-size']*3)

            args = []
            arg_pointer = untagged_pointer
            for i in range(num_ptrs + num_non_ptrs):
                arg_pointer = ptrutil.pointer_offset(settings, arg_pointer, settings['word-size']);
...
def read_case(settings, parsed, pointer, stack, scrutinee):
    try:
        ...
        mach = machine.Machine(settings, parsed, copy.deepcopy(stack), {
            settings['main-register']: CaseArgument(inspection = pointer),
            settings['stack-register']: StackPointer(index = -len(stack))
        })
        ...
        if first_instructions[-2].mnemonic == 'cmp' and first_instructions[-2].operands[0].type == capstone.x86.X86_OP_REG and machine.base_register(first_instructions[-2].operands[0].reg) in mach.registers and isinstance(mach.registers[machine.base_register(first_instructions[-2].operands[0].reg)], CaseArgument) and first_instructions[-2].operands[1].type == capstone.x86.X86_OP_IMM:
            ...
            read_code(settings, parsed, pointer, stack, {settings['main-register']: CaseArgument(inspection = pointer)})
...
def read_function_thunk(settings, parsed, pointer, main_register, num_args):
    ...
    extra_stack = []
    registers = {}
    registers[settings['main-register']] = main_register
    for i in range(num_args):
        if i < len(settings['arg-registers']):
            registers[settings['arg-registers'][i]] = Argument(index = i, func = info_name)
        else:
...
def read_code(settings, parsed, pointer, extra_stack, registers):
    try:
        ...
        registers[settings['heap-register']] = HeapPointer(heap_segment = pointer, index = -1, tag = 0)
        registers[settings['stack-register']] = StackPointer(index = -len(extra_stack))
        mach = machine.Machine(settings, parsed, extra_stack, registers)
        mach.simulate(instructions)

        stack = mach.stack[registers[settings['stack-register']].index:]
        ...
        if instructions[-1].operands[0].type == capstone.x86.X86_OP_MEM and machine.base_register(instructions[-1].operands[0].mem.base) == settings['stack-register']:
            if settings['opts'].verbose:
                print("    Interpretation: return", show.show_pretty(settings, registers[settings['main-register']]))
                print()

            parsed['interpretations'][pointer] = Pointer(registers[settings['main-register']])

            read_closure(settings, parsed, registers[settings['main-register']])
        else:
            worklist = []
            uses = []

            if instructions[-1].operands[0].type == capstone.x86.X86_OP_MEM:
                assert machine.base_register(instructions[-1].operands[0].mem.base) == settings['main-register']
                assert instructions[-1].operands[0].mem.disp == 0

                if settings['opts'].verbose:
                    print("    Interpretation: evaluate", show.show_pretty(settings, registers[settings['main-register']]))

                stack_index = 0
                interpretation = Pointer(registers[settings['main-register']])
                worklist.append({'type': 'closure', 'pointer': registers[settings['main-register']]})
            elif instructions[-1].operands[0].type == capstone.x86.X86_OP_IMM:
                jmp_address = instructions[-1].operands[0].imm
                if jmp_address in settings['address-to-name'] and settings['address-to-name'][jmp_address][:7] == 'stg_ap_':
                    func = settings['address-to-name'][jmp_address]
                    if func.split('_')[2] == '0':
                        num_args = 0
                        arg_pattern = ''
                    else:
                    ...
                    called = registers[settings['main-register']]
                    ...
                    worklist.append({'type': 'function/thunk', 'pointer': called, 'main-register': registers[settings['main-register']], 'num-args': num_args})
                    func_type = 'info'
                ...
                for reg, i in zip(settings['arg-registers'], range(num_args)):
